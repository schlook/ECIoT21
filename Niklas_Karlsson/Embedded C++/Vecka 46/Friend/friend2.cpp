...
public:
    ...
    friend void ränteberäknaren(bankkonto *lista, int antal);
...
int main(void)
{
    ...
    bankkonto *allemansKonto;
    allemansKonto = new bankkonto;

    ränteberäknaren(allemansKonto, 5);
...
void ränteberäknaren(bankkonto *lista, int antal)
{
    for (int i = 0; i < antal; i++)
    {
        lista[i].saldo = rand() + 1;
        lista[i].saldo += lista[i].saldo * lista[i].ränta;
        std::cout << "Med nuvarande ränta är saldot på kontot: " << lista[i].saldo << std::endl;
    }
}